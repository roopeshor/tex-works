...
\usepackage[margin={1.6cm,2.3cm}]{geometry}
\usepackage{float}
\usepackage{titlesec}
\usepackage{enumitem}
\usepackage{graphicx}
...
\begin{enumerate}[leftmargin=*, itemsep=15pt]
	...
	\item \textbf{Plotting Magnetic Field on Remaining Faces}
		...
		\end{enumerate}
    \item \textbf{Obtaining Field Pattern for different Inputs}
        \begin{enumerate}
            \item Right click \textbf{Excitations $\rightarrow$ Port Excitations}
            \item Change amplitude of each ports and the phase relative to integration line.
            \item Observe the pattern
        \end{enumerate}
\end{enumerate}
\section*{Result}
