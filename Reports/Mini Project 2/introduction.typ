...
- One _*Originating Transmitter*_: Simulating an edge device triggered by a user in distress.
- One _*Destination Receiver*_: Acting as the gateway or rescue command center.
...
#figure(
	image("images/mesh-impl.svg", height: 5cm),
	caption: [Intended organizationo of nodes]
)<mesh-impl>

== Protocol Design

A primary decision in this project was the intentional bypass of heavy, conventional mesh routing protocols like #acr("AODV") or Zigbee. While these protocols are highly efficient in static, enterprise-grade networks, they require significant effort to get started and to maintain routing tables and calculate shortest-path vectors. Since no #acs("GNSS") system is incoorporated the node distance is not directly computable.

Instead, Controlled Flooding is implementated. In a flooding architecture, an intermediate node simply broadcasts any packet it receives that is not addressed to itself. To prevent the network from collapsing under infinite routing loops (known as a "broadcast storm"), the flooding is strictly controlled via software. Each node maintains a localized "seen stack" -- an array of details of recently transmitted packets. If a packet's signature exists in the stack, it is dropped; if it is new, it is forwarded. This ensures redundancy and that an SOS signal will aggressively find the fastest physical path through the forest to the receiver, without the nodes needing to map the network topology in advance.

In emergency communications, "fire-and-forget" transmission is unacceptable; the originating node must know if the distress signal successfully reached the command station. To signal the successful data delivery, the system implements an application-layer #acr("ACK") mechanism. When the final destination node successfully receives the targeted packet, it generates a distinct #acr("ACK") packet and routes it back through the mesh to the original transmitter. The originating node after transmitting SOS goes to listening state, and upon receiving this targeted #acr("ACK"), the node provides physical feedback (via an LED indicator) to confirm that the transmission successfully reached the command station.