...
\usection{Differential equation (DE)}
...
\ussubsection{\texorpdfstring{1$^{\text{st}}$ order differential equation}{1st order differential equation}}
\begin{flalign*}
...
\ussubsection{\texorpdfstring{1$^{\text{st}}$ Order Linear Differential equation}{1st Order Linear Differential equation}}
General form:
\begin{flalign*}
...
\qa{2}{Solve\ \ $\dfrac{dy}{dx} + 2y\tan(x)=\sin(x)$}{
	...
	\begin{flalign*}
		\text{Integrating factor (IF) } F & =e^{\int P \cdot dx}                                                                                 & \\
		                                  & =e^{\int 2 \tan(x) dx}                                                                                 \\
		                                  & =e^{2 \cdot \ln |\sec (x)|}       \hspace*{3cm} \boxed{\smallint \tan(x)dx =\ln\left|\sec(x)\right|}   \\
...
\end{flalign*}
\eqnleft{P(x)=-x^{-1}, Q(x)=x^2 \cos(2 x)}
\eqnleft{\therefore I.F =e^{\int p(x) \cdot dx}=e^{-\int x^{-1}dx}=e^{-\ln(x)}=x^{-1}=\frac{1}{x}}
...
\begin{flalign*}
	...
	\therefore y        & =\frac{x^2 \cdot \sin(2 x)}{2}+\frac{x \cos(2 x)}{4}+x C \tag*{(1)} \label{eq:3y}
\end{flalign*}
...
IF = $e^{\int P x}=e^{\int -2 / x \cdot d x}=e^{-2 \ln (x)}=x^{-2}$
...
\end{flalign*}
\eqnleft{P(x)=\frac{2}{x}, Q(x)=\frac{\cos(x)}{x^2 },\ \text{IF }=e^{\int P \cdot dx} = e^{\int 2/x\ dx}=e^{\ln \left(x^2 \right)}=x^2}
...
\begin{flalign*}
	y \cdot x^2 & =\int x^2 \cdot \frac{\cos(x)}{x^2 } \cdot dx+C           & \\
	            & =\int \cos(x) dx+C                                          \\
	y \cdot x^2 & =\sin(x)+C                                                  \\
	            & \Rightarrow \boxed{y=\frac{\sin(x)}{x^2 }+\frac{C}{x^2 }}   \\
\end{flalign*}

}

\qa{7}{Solve $y'+\frac{2 y}{x}=\frac{4}{x}$. where $y(1) = 6$}{

\eqnleft{P(x)=\dfrac{2}{x}, Q(x)=\dfrac{4}{x},\ \ \text{IF }=e^{\int P dx}=e^{\int 2 / x \cdot dx}=x^2}
...
\usection{Variable Separable Equation}
