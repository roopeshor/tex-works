\begin{center}
	...
	\begin{tikzpicture}[decoration={snake}, scale=.9]
		...
		\begin{scope}[xshift=9cm,decoration={snake}]
			...
			\draw[-{Latex[length=3mm]}] ($(E2t)!.5!(Ee2)$) coordinate (x1) -- ($($(E1t)!.5!(Ee1)$)+(0,.15)$) coordinate (x2);
			\draw[thick,decorate,red,->] ($($(x1)!.5!(x2)$) + (.3, 0)$) coordinate (y1) -- node [above,align=left,yshift=1mm,font=\footnotesize] {$h\nu_{12}$} ($(y1) + (1,0)$);
			\draw[thick,decorate,red,->] ($(y1) + (0,-.3)$) coordinate (y1) -- node [below,align=left,font=\footnotesize] {$h\nu_{12}$} ($(y1) + (0:1)$);
